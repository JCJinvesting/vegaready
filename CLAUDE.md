...
7. Reading model: Analyst `/markets/<desk>` (dense master) vs Layman `/layman/<desk>` (independent, simpler) via `data-mode`; Skim/Deep within a page via `data-disc`. Per-desk palette via a `skin-*.css` + `data-skin` on the route. Follow `_program/artifacts/Typography-Emphasis-Spec.md` for all formatting.
