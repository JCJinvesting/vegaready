...
- **CW4 · Layman Deep + chapter system. ✅ DONE (2026-06-14).** Built across micro-loops L1–L8: `chnav` (9 numeral pills) + always-visible `laych` dividers; reorganized the 11 `ov*` sections + 3 net-new R-08 chapters (V factors / VI plumbing / VII rules-break) into the 9-chapter taxonomy; brass **chapter accents** `--chA..--chI` (warm-earthen sweep, owner-picked Option A). Data-wired anchored panels + signature SVG figures (cycle, spread thermometer, ratings ladder, plumbing flow, correlation flip, scenario map); Ch IX honest "what we can't show live" panel. Plan: `docs/VegaReady-Credit-Layman-Deep-Plan.md`.
- **CW5 · Layman Skim (designed tier). ✅ DONE (2026-06-14).** `skbanner` + `sk60` ("Credit in 60 seconds", data-wired) + `skdigest` (9-line chapter digest) + per-chapter `skim-bl` "in short" on the all-deep chapters + remember-three + goDeep wiring. Compresses the complete Layman Deep; verified skim shows / deep hides via `offsetParent` measurement. Residual-Skim problem killed.
- **CW6 · Cross-tier polish. ✅ DONE (folded into the loops + the visual-QC pass).** Pull-quotes, semantic tints, accent beats applied per chapter; final graphical QC refined the cycle loop-back arrow + plumbing buffer contrast.
- **CW7 · QC + ship. ✅ DONE — SHIPPED to production 2026-06-14 (merge `07781aa`).** curl-verified all four tiers (chapters render, both Skims complete, no escaped HTML, layman no analyst-leak, trailing-slash URLs, sibling tier intact); merge grepped before push; production verified. **The credit desk is complete.** Generalized into `docs/VegaReady-Desk-Build-Playbook.md`.
